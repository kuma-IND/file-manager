...
$(document).ready(function () {
    ...
    $(document).on("keypress", "#memory_limit", function (e) {
    ...
    $(document).on("click", "#confirm_user_delete", function () {
        deleteUser($(this).attr("data-user_id"));
    });
    ...
    $(document).on("click", ".delete_file", function () {
        openConfirmFileDeleteModal($(this));
    });

    $(document).on("click", ".delete_user", function () {
        openConfirmUserDeleteModal($(this));
    });
...
function isPasswordFormValid() {
    let errors = [];
    if (!$("#change_password_form #current_user_password").val().trim().length) {
        errors.push("Please provide current password");
    }

    if (!$("#change_password_form #user_password").val().trim().length) {
        errors.push("Please provide new password");
    } else {
        let password_1 = $("#change_password_form #user_password").val().trim();
        let password_2 = $("#change_password_form #user_password_2").val().trim();

        if (password_1 !== password_2) {
            errors.push("New password and repeat password do not match");
        }
    }

    if (errors.length) {
        showAlert(setError(errors.join("<br>")));
        return false;
    } else {
        return true;
    }
...
function isUserFormValid() {
    let errors = [];
    if (!$("#add_edit_form #user_name").val().trim().length) {
        errors.push("Name can't be ampty");
    }

    if (!isValidEmail($("#add_edit_form #user_email").val().trim())) {
        errors.push("Please enter valid email address");
    }

    if ($("#add_edit_form #memory_limit").length) {
        if (!$("#add_edit_form #memory_limit").val().trim().length) {
            errors.push("Memory limit can't be empty");
        } else if (isNaN($("#add_edit_form #memory_limit").val().trim())) {
            errors.push("Please enter correct memory limit");
        }
    }
...
function getUserForm(json_ob, id) {
    ...
    switch (json_ob.operation) {
        ...
        case "edit":
            let valid = true;
            if (json_ob.hasOwnProperty("operation_type") && Number(json_ob.usertype_id) !== 1) {
                valid = false;
            }
...
function addEditUser(ele) {
    ...
    if ($("#add_edit_form #memory_limit").length) {
        data["memory_limit"] = $("#add_edit_form #memory_limit").val()
    }
...
function displayUserTable(json_ob) {
    ...
    $.each(json_ob, function (i, row) {
        ...
        if (Number(row.id) !== Number(logged_in_object.id)) {
...
function openConfirmFileDeleteModal(ele) {
    let color = "danger";
    ...
    gui += "Do you really want to delete file?";
...
function openConfirmUserDeleteModal(ele) {
    let color = "danger";
    var modal_id = "user_delete_modal";
    ...
    gui += "Do you really want to delete user?";

    gui += "</div>";
    gui += "<div class='modal-footer'>";
    gui += "<button type='submit' id='confirm_user_delete' data-user_id='" + ele.closest("tr").attr("data-id") + "' class='btn btn-" + color + "'>Yes</button>";
    gui += "<button type='button' class='btn btn-default' data-dismiss='modal'>No</button>";
    gui += "</div>";
    gui += "</div>";
    gui += "</div>";
...
function uploadFiles(index) {
    if ($("[data-selected_file_index='" + index + "'] .progress_cell").text() === "") {
            ...
            xhr: function () {
                ...
                xhr.upload.addEventListener('progress', function (ev) {
                    if (ev.lengthComputable) {
                        let form_modal = $("#image_filter_modal");
...
function deleteUser(user_id) {
    $.ajax({
        ...
        success: function (response) {
            var json_ob = JSON.parse(response);
            if (json_ob !== false) {
                destroyModal($("#user_delete_modal"));
                displayUserTable(json_ob.users);
                showAlert(json_ob.message);
            }
        },
...
//# sourceMappingURL=index-89364bff26.js.map
