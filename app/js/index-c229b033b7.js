        ...
        xhr: function () {
            ...
            xhr.upload.addEventListener('progress', function (ev) {
                if (ev.lengthComputable) {
                    ...
                    if (form_modal.find(".modal-body .progress").length === 0) {
                        form_modal.find(".modal-body").append(progress_bar);
                    }

                    var percentage = Math.round(ev.loaded / ev.total * 100);
                    changeProgressBarStatus(form_modal.find(".modal-body .progress-bar"), percentage);
                    if (percentage === 100) {
                        savingProgressBarStatus(form_modal.find(".modal-body .progress-bar"));
                    }
                }
            });
            return xhr;
...
function updateSelectedFileTable(files) {
    ...
    $.each(selected_files, function (i, selected_file) {
        rows += "<tr>";
        rows += "<td class='small_cell'>" + (i + 1) + "</td>";
        rows += "<td>" + selected_file.name + "</td>";
        rows += "<td class='small_cell'>" + selected_file.size + "</td>";
        rows += "</tr>";
    });
    ...
    rows += "<tr>";
    rows += "<th class='small_cell'>" + (i + 1) + "</th>";
    rows += "<th>" + selected_file.name + "</th>";
    rows += "<th class='small_cell'>" + selected_file.size + "</th>";
    rows += "</tr>";
    table += "</table>";
    table += "</div>";
...
//# sourceMappingURL=index-c229b033b7.js.map
